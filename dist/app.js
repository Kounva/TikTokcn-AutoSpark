...
const Layout = {
  ...
  <div class="app-shell">
    ...
    <aside class="sidebar" :class="{ collapsed: isCollapsed && !isMobile, 'sidebar-mobile': isMobile, 'sidebar-mobile-visible': sidebarVisible }">
      ...
      <div class="sidebar-footer">
        <a href="https://github.com/DkoBot/TikTokAutoSparkWeb" target="_blank" style="color:var(--text-muted);text-decoration:none;font-size:12px;display:flex;align-items:center;gap:6px">
      ...
      <header class="topbar">
        ...
        <div class="topbar-right">
          <div class="topbar-user">
            <div class="topbar-avatar">{{ (userStore.userInfo.username || 'A')[0].toUpperCase() }}</div>
            <span class="topbar-username">{{ userStore.userInfo.username || 'Admin' }}</span>
          </div>
  ...
  setup() {
    ...
    return { isCollapsed, isMobile, sidebarVisible, toggleSidebar, closeSidebar, menuList, activeMenu, currentMenuTitle, handleMenuSelect, handleCommand, userStore, browserStatus };
  }
};
...
const Home = {
  ...
  setup() {
    ...
    const checkLoginStatus = async () => { try { const res = await API.getLoginStatus(); loginStatus.value = res.data === 'Yes'; setLoginStatus(loginStatus.value); } catch (e) { loginStatus.value = false; setLoginStatus(false); } };
...
const Tasks = {
  ...
  <div class="glass-shell anim-fade-up">
    <div class="glass-core">
      ...
      <el-dialog v-model="dialogVisible" :title="dialogMode === 'add' ? '添加定时任务' : '修改执行时间'" width="500px" destroy-on-close>
...
const Settings = {
  ...
  setup() {
    ...
    const handleRefreshStatus = async () => { refreshStatusLoading.value = true; try { usernameLoaded.value = false; localStorage.removeItem('douyin_username'); localStorage.removeItem('douyin_username_loaded'); await checkLoginStatus(); await fetchLastLoginIP(); ElMessage.success(loginStatus.value ? '已登录' : '未登录'); } finally { refreshStatusLoading.value = false; } };
    ...
    const handleCheckLogin = async () => { checkLoading.value = true; try { const res = await API.pnglogin(); loginStatus.value = res.code == 200; setLoginStatus(loginStatus.value); if (loginStatus.value) { ElMessage.success('登录成功'); qrDialogVisible.value = false; username.value = ''; usernameLoaded.value = false; localStorage.removeItem('douyin_username'); localStorage.removeItem('douyin_username_loaded'); await fetchUsername(); await fetchFriendsList(); } else ElMessage.warning('未登录，请继续扫码'); } catch (e) { ElMessage.error('扫码登录失败，请重试'); } finally { checkLoading.value = false; } };
    ...
    const handleManualLogin = async () => { if (!manualForm.value.cookie.trim()) { ElMessage.warning('请输入Base64Cookie'); return; } manualLoading.value = true; try { const res = await API.login(manualForm.value.cookie); if (res.data === 'ok') { ElMessage.success('登录成功'); loginStatus.value = true; setLoginStatus(true); manualDialogVisible.value = false; username.value = ''; usernameLoaded.value = false; localStorage.removeItem('douyin_username'); localStorage.removeItem('douyin_username_loaded'); await fetchUsername(); await fetchFriendsList(); } else ElMessage.error('登录失败，Cookie无效'); } catch (e) { ElMessage.error('登录失败，请检查Cookie'); } finally { manualLoading.value = false; } };
    ...
    const handleDieLogin = async () => { try { await API.dieLogin(); setLoginStatus(false); localStorage.removeItem('douyin_token'); ElMessage.success('已强制退出登录'); } catch (e) { ElMessage.error('强制退出失败'); } };
    ...
    const handlePhoneLogin = async () => { if (!phoneForm.value.phone) { ElMessage.warning('请输入手机号'); return; } if (!phoneForm.value.code) { ElMessage.warning('请输入验证码'); return; } phoneLoading.value = true; try { const res = await API.submitVerifyCode(phoneForm.value.code); if (res.code == 200) { ElMessage.success('登录成功'); phoneDialogVisible.value = false; setLoginStatus(true); username.value = ''; usernameLoaded.value = false; localStorage.removeItem('douyin_username'); localStorage.removeItem('douyin_username_loaded'); await fetchUsername(); } else ElMessage.error(res.data || '登录失败'); } catch (e) { ElMessage.error('登录失败，请重试'); } finally { phoneLoading.value = false; } };
